...
- **The first two environment-variable secrets moved — 13 September 2026.** `OIDC_<ID>_CLIENT_SECRET` and `SMTP_PASS` are now names resolved through `getSecret` at the moment of use (the token exchange; an outbox mail send), with the environment variable's value read fresh from the environment at that same moment — never snapshotted onto `ServerConfig`, which is reachable from every route handler for the whole deprecation window — when the store holds nothing under the name, and warned about once at boot. `pnpm --filter @quill/server secrets:set <name>` (beside `secrets:rotate`; no package declares a `bin`, so this is the command, not `quill secrets:set`) is how an administrator enters one: the value is read from stdin, never from `argv`, and the command refuses a second argument and a terminal stdin outright rather than accepting the one or hanging on the other — the recipe every document gives is `< /path/to/secret-file` or a shell variable already in memory, never the value typed on the command line itself. Boot validation checks that a secret's name resolves to something *readable* — the store, with its `keyId` one this instance's `KeyProvider` still holds, or the fallback — without ever opening a ciphertext to find out (`describeSecret`, not `getSecret`); a row wrapped by a since-rotated-away master key fails this the same as a missing secret, rather than passing silently and failing later at a sign-in or a mail send. This is the template the remaining environment-variable secrets follow when their turn comes.

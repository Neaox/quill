...
- Anonymous requests to public routes receive server-rendered HTML from the same React reading components, cached by content hash and invalidated by publish events.
...
- The authenticated application remains a single-page app. Whether the SSR path is a Fastify-hosted React render or TanStack Start is decided by a spike in the research phase; the reading components are shared either way.
...
- Public publishing: public principal, publish settings, SSR reading pages, sitemap, metadata, public search.
...
- OIDC sign-in.
- Locks in: the anonymous principal and the SSR path.
